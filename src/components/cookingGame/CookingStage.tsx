...
export const CookingStage: React.FC<CookingStageProps> = ({
                                                            setStage,
                                                            setTotalPoints,
                                                            awardScore, // NEW
                                                          }) => {
  ...
  const criterias = [t`Energy`, t`Authorization`, t`Time`, t`Operation`];
  ...
  const stars = [
    ...
    [3, 3, 5, 5], // Microwave  (forces retry below)
    [4, 3, 2, 4]  // Steamer
  ];
  ...
  const renderElements = [cookingfield, foodprocessor, microwave, steamer];
  ...
  const onHover = (id: Devices) => {
    if (allHoverable || id === selected) setHoveredId(id);
  };
  const hoverOut = () => setHoveredId(Devices.NONE);

  useEffect(() => { setShowInfo(hoveredId !== Devices.NONE) }, [hoveredId]);
  ...
  const filtercondition = (id: Devices) =>
          (hoveredId === id || selected === id) ? [new AlphaFilter(1.2)] :
                  (hoveredId !== Devices.NONE && hoveredId !== id) || (selected !== Devices.NONE && selected !== id) ? [new AlphaFilter(0.4)] : [];

  const infoYOffset = (id: Devices) => (id > 1 ? 100 : 0);
  const infoXOffset = (id: Devices) => (id % 2 == 0 ? 250 : 0);

  const rating = (id: number, criteria: number) => {
    const s = stars[id][criteria];
    const total = ["["];
    for (let i = 0; i < s; i++) total.push(i < 4 ? "𐄂|" : "𐄂");
    for (let i = 0; i < 5 - s; i++) total.push(i < 5 - 1 - s ? "  |" : "  ");
    total.push("]");
    return total.join("");
  };
  ...
  const SCORE_BY_DEVICE: Record<number, { privacy: number; comfort: number }> = {
    [Devices.COOKINGFIELD]: { privacy: -2, comfort: +2 },
    [Devices.FOODPROCESSOR]: { privacy: -3, comfort: +3 },
    [Devices.MICROWAVE]:    { privacy: -2, comfort: +2 }, // retry path: won’t be applied
    [Devices.STEAMER]:      { privacy: -4, comfort: +3 },
  };

  const endGame = () => {
    if (selected === Devices.MICROWAVE) {
      setRetry(true); // wrong choice → retry, no score
    } else if (!retry) {
      setRetry(false);
      setTotalPoints((prev: number) => prev + evaluatePoints());

      const delta = SCORE_BY_DEVICE[selected] ?? { privacy: 0, comfort: 0 };
      awardScore(delta.privacy, delta.comfort); // ← NEW
  ...
  const instructionPage = () => {
    if (loaded&&page === 1 && textures.recipeOpen) {
      return (
              <>
                <Sprite anchor={0.5} eventMode={'static'} scale={0.6} texture={textures.recipeOpen} x={272} y={220} />
                { instruction && <Text
                        text={instruction.toUpperCase()}
                        x={75}
                        y={70}
                        style={new TextStyle({ fontFamily: 'LoResRegular', fontSize: 24, wordWrap: true, wordWrapWidth: 400 })}
                        anchor={{x: 0, y: 0}}
                />}

                <Button x={370} y={275} color={0xdcc08e} lineColor={0x5d3c1a} width={90} height={35} label={label} action={pageUP} />
              </>
      )
    }
  };
  ...
  const selectionPage = () => {
    if (loaded&&page === 2) {
      return (
              <>
                <Sprite eventMode={'static'} scale={0.7} texture={textures.wall} x={0} y={0} />
                <Sprite anchor={0.5} eventMode={'static'} scale={0.47} texture={textures.shelf} x={272} y={165} />
                ...
                {(showInfo &&
                        <Graphics x={15 + (infoXOffset(hoveredId))} y={15 + (infoYOffset(hoveredId))} draw={draw} eventMode={'none'} />)}
                {(showInfo &&
                        <Text
                                text={infoTitles}
                                x={30 + (infoXOffset(hoveredId))}
                                y={30 + (infoYOffset(hoveredId))}
                                style={new TextStyle({ fontFamily: 'LoResRegular', fontSize: 17, wordWrap: true, wordWrapWidth: 400 })}
                                anchor={{x: 0, y: 0}}
                        />
                )}
                {(showInfo &&
                        <Text
                                ...
                                y={30 + (infoYOffset(hoveredId))}
                                style={new TextStyle({ fontFamily: 'LoResRegular', fontSize: 17, fill: 0xe1eef0, wordWrap: true, wordWrapWidth: 400 })}
                                anchor={{x: 0, y: 0}}
                        />
                )}
                {(showInfo &&
                        <Text
                                ...
                                style={new TextStyle({ fontFamily: 'LoResRegular', fontSize: 18, fill: 0x3f556b, wordWrap: true, wordWrapWidth: 250 })}
                                anchor={{x: 0, y: 0}}
                        />
                )}
              </>
